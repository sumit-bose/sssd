...
struct _sbus_sss_invoke_in__out_iss_state {
    struct _sbus_sss_invoker_args_iss out;
    struct {
        enum sbus_handler_type type;
        void *data;
        errno_t (*sync)(TALLOC_CTX *, struct sbus_request *, void *, int32_t*, const char **, const char **);
        struct tevent_req * (*send)(TALLOC_CTX *, struct tevent_context *, struct sbus_request *, void *);
        errno_t (*recv)(TALLOC_CTX *, struct tevent_req *, int32_t*, const char **, const char **);
    } handler;
...
static void
_sbus_sss_invoke_in__out_iss_step
    (struct tevent_context *ev,
     struct tevent_timer *te,
     struct timeval tv,
     void *private_data);
...
static void
_sbus_sss_invoke_in__out_iss_done
   (struct tevent_req *subreq);
...
_sbus_sss_invoke_in__out_iss_send
   (TALLOC_CTX *mem_ctx,
    ...
    const char **_key)
{
    struct _sbus_sss_invoke_in__out_iss_state *state;
    struct tevent_req *req;
    const char *key;
    errno_t ret;

    req = tevent_req_create(mem_ctx, &state, struct _sbus_sss_invoke_in__out_iss_state);
    if (req == NULL) {
        DEBUG(SSSDBG_CRIT_FAILURE, "Unable to create tevent request!\n");
        return NULL;
    }
    ...
    ret = sbus_invoker_schedule(state, ev, _sbus_sss_invoke_in__out_iss_step, req);
    if (ret != EOK) {
        goto done;
    }
...
static void _sbus_sss_invoke_in__out_iss_step
   (struct tevent_context *ev,
    struct tevent_timer *te,
    struct timeval tv,
    void *private_data)
{
    ...
    req = talloc_get_type(private_data, struct tevent_req);
    state = tevent_req_data(req, struct _sbus_sss_invoke_in__out_iss_state);
    ...
    case SBUS_HANDLER_SYNC:
        ...
        ret = state->handler.sync(state, state->sbus_req, state->handler.data, &state->out.arg0, &state->out.arg1, &state->out.arg2);
        if (ret != EOK) {
            goto done;
        }

        ret = _sbus_sss_invoker_write_iss(state->write_iterator, &state->out);
        goto done;
    case SBUS_HANDLER_ASYNC:
        if (state->handler.send == NULL || state->handler.recv == NULL) {
            DEBUG(SSSDBG_CRIT_FAILURE, "Bug: async handler is not specified!\n");
        ...
        tevent_req_set_callback(subreq, _sbus_sss_invoke_in__out_iss_done, req);
        ret = EAGAIN;
        goto done;
    }
...
static void _sbus_sss_invoke_in__out_iss_done(struct tevent_req *subreq)
{
    struct _sbus_sss_invoke_in__out_iss_state *state;
    struct tevent_req *req;
    errno_t ret;
